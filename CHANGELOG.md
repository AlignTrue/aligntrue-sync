...
- **Import flow messaging** - Clarified init and add command messaging. Now shows "Found X rules from [source]" before confirmation instead of confusing "Import complete" followed by "Initialize with X rules?" Also improved error messages to include supported source formats with examples and points to https://sync.aligntrue.ai/sources
