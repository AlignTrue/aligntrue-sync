...
test -f .aligntrue/rules/scopes-demo.md || { echo "✗ scopes-demo.md missing"; exit 1; }
...
grep -q "scopes:" .aligntrue/rules/scopes-demo.md || { echo "✗ scopes section missing"; exit 1; }

# Count scopes (should be 3)
SCOPE_COUNT=$(grep -c 'path: "apps/web"\|path: "packages/api"\|path: "services/worker"' .aligntrue/rules/scopes-demo.md || true)
...
grep -q 'path: "apps/web"' .aligntrue/rules/scopes-demo.md || { echo "✗ apps/web scope missing"; exit 1; }
grep -q 'path: "packages/api"' .aligntrue/rules/scopes-demo.md || { echo "✗ packages/api scope missing"; exit 1; }
grep -q 'path: "services/worker"' .aligntrue/rules/scopes-demo.md || { echo "✗ services/worker scope missing"; exit 1; }

# Check scope properties
echo "✓ Checking scope properties..."
grep -q "include:" .aligntrue/rules/scopes-demo.md || { echo "✗ include patterns missing"; exit 1; }
grep -q "exclude:" .aligntrue/rules/scopes-demo.md || { echo "✗ exclude patterns missing"; exit 1; }
grep -q "rulesets:" .aligntrue/rules/scopes-demo.md || { echo "✗ rulesets missing"; exit 1; }

# Check merge configuration
echo "✓ Checking merge configuration..."
grep -q "merge:" .aligntrue/rules/scopes-demo.md || { echo "✗ merge configuration missing"; exit 1; }
grep -q 'strategy: "deep"' .aligntrue/rules/scopes-demo.md || { echo "✗ merge strategy missing"; exit 1; }
