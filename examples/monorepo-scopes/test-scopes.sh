...
# Check scopes section exists in rules.md
echo "✓ Checking scopes section..."
grep -q "scopes:" .aligntrue/rules.md || { echo "✗ scopes section missing"; exit 1; }

# Count scopes (should be 3)
SCOPE_COUNT=$(grep -c 'path: "apps/web"\|path: "packages/api"\|path: "services/worker"' .aligntrue/rules.md || true)
...
grep -q 'path: "apps/web"' .aligntrue/rules.md || { echo "✗ apps/web scope missing"; exit 1; }
grep -q 'path: "packages/api"' .aligntrue/rules.md || { echo "✗ packages/api scope missing"; exit 1; }
grep -q 'path: "services/worker"' .aligntrue/rules.md || { echo "✗ services/worker scope missing"; exit 1; }

# Check scope properties
echo "✓ Checking scope properties..."
grep -q "include:" .aligntrue/rules.md || { echo "✗ include patterns missing"; exit 1; }
grep -q "exclude:" .aligntrue/rules.md || { echo "✗ exclude patterns missing"; exit 1; }
grep -q "rulesets:" .aligntrue/rules.md || { echo "✗ rulesets missing"; exit 1; }

# Check merge configuration
echo "✓ Checking merge configuration..."
grep -q "merge:" .aligntrue/rules.md || { echo "✗ merge configuration missing"; exit 1; }
grep -q 'strategy: "deep"' .aligntrue/rules.md || { echo "✗ merge strategy missing"; exit 1; }
