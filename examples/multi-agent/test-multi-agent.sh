...
grep -q "vendor:" .aligntrue/rules.md || { echo "✗ vendor bags missing"; exit 1; }

# Count vendor bag instances (should be multiple)
VENDOR_COUNT=$(grep -c "vendor:" .aligntrue/rules.md || true)
...
grep -q "cursor:" .aligntrue/rules.md || { echo "✗ cursor vendor namespace missing"; exit 1; }
grep -q "claude:" .aligntrue/rules.md || { echo "✗ claude vendor namespace missing"; exit 1; }
grep -q "copilot:" .aligntrue/rules.md || { echo "✗ copilot vendor namespace missing"; exit 1; }

# Check vendor bag properties
echo "✓ Checking vendor bag properties..."
grep -q "ai_hint:" .aligntrue/rules.md || { echo "✗ ai_hint property missing"; exit 1; }
grep -q "quick_fix:" .aligntrue/rules.md || { echo "✗ quick_fix property missing"; exit 1; }
grep -q "priority:" .aligntrue/rules.md || { echo "✗ priority property missing"; exit 1; }

# Check rules exist
echo "✓ Checking rules..."
RULE_COUNT=$(grep -c "  - id: " .aligntrue/rules.md || true)
