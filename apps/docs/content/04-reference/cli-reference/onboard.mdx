...
description: "Generate personalized onboarding checklists with `aln onboard`: setup, usage, and customization."
---
  ...
  onboard:
    ...
    steps:
      ...
      - name: Run checks (generate SARIF)
        run: aln drift --sarif > checks.sarif
...
onboard:
  ...
  script:
    ...
    - aln drift --sarif > checks.sarif || true
