...
In team mode, rules can be marked as "personal" (using `scope: personal`) to exclude them from lockfile validation and team approval workflows.
...
For practical workflows and detailed examples, see [Rule Visibility Guide](/docs/01-guides/09-rule-visibility).

For conceptual background on approval scopes and storage options, see [Rule Visibility Concepts](/docs/03-concepts/rule-visibility).
