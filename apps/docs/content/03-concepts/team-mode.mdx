...
> Routing differs in solo mode: all rules go to `remotes.personal` unless a rule is `scope: shared` and a shared remote exists. See [Rule sharing & privacy](/docs/01-guides/06-rule-sharing-privacy) for full mode-based routing.
