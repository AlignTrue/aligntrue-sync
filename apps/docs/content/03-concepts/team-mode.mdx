...
<Callout type="warning" title=".aligntrue.bundle.yaml status">
  ...
  **not** write `.aligntrue.bundle.yaml` yet. Keep the flag enabled for forward
  compatibility; expect to continue committing `.aligntrue/.rules.yaml` and
...
4. Preserves `.aligntrue/.rules.yaml` (team rules become solo public rules)
...
- `sync.auto_pull` - Whether to automatically pull agent changes
- `sync.workflow_mode` - Workflow mode (ir_first or native_format)
- `sync.primary_agent` - Primary agent for sync operations
...
aligntrue sync
git add .aligntrue.lock.json
git commit -m "chore: Add new pack"
...
Add `scope: personal` to section frontmatter in `.aligntrue/.rules.yaml`:
...
3. Mark sections as personal in `.aligntrue/.rules.yaml`
...
# Mark as personal in .aligntrue/.rules.yaml
...
With centralized rule management enabled (default):

- Personal sections sync from your edit_source
...
| Syncs from agent    | No                    | Yes (if edit_source) |
