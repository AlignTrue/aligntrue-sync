...
- **`.aligntrue/config.team.yaml`** (new, committed) - Team settings: `mode: team`, lockfile, bundle, shared sources and exporters
...
Use `aligntrue sync --dry-run` to preview the impact of a change. Otherwise commit the regenerated lockfile whenever hashes change—this is the expected signal that your IR evolved.
...
- `mode: team`
- `modules.lockfile`, `modules.bundle`
- `lockfile.*` configuration
- Shared `sources` and `exporters`
...
- `sources`, `exporters` - Both files contribute to the final list
...
To approve sources:
  Add to .aligntrue/config.team.yaml and commit via PR
...
1. **Intended source:** Add to config and commit via PR
   ...
   ```bash
   # Edit .aligntrue/config.team.yaml to add source
   aligntrue sync
   git add .aligntrue/lock.json
