...
- Developer edited `AGENTS.md` directly but didn't run `aligntrue sync --accept-agent`
- Copy/pasting new rules into agent files

**How to fix:**

```bash
# If changes are intended (pull back to IR):
aligntrue sync --accept-agent agents

# If changes are accidental (overwrite with IR):
aligntrue sync
```
