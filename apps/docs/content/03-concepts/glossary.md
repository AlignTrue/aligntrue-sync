...
The user-facing name for a pack created in the Align Catalog. An Align pack bundles multiple Aligns (by ID) into a single downloadable/shareable unit with preserved paths. Install via CLI with `aligntrue add <pack-id>` or `aligntrue add https://sync.aligntrue.ai/a/<pack-id>`. Internal references to "catalog pack" refer to the same pack type.
