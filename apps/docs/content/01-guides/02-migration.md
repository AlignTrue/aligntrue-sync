...
Done! Your rules are now managed by AlignTrue and synced to all agents.
...
AlignTrue imports all `.mdc` files and merges them into a single source.
...
- Edit rules in markdown instead of MDC format
...
.aligntrue/
  rules.md          # Single source of truth
  config.yaml
AGENTS.md           # Generated by AlignTrue (preserved)
.cursor/
  rules/
...
.aligntrue/
  rules.md          # Single source of truth
  config.yaml
AGENTS.md           # Generated (Aider reads this)
.aider.conf.yml     # Preserved if needed
```

**Note:** Aider reads AGENTS.md, so AlignTrue generates that format.
...
```
.aligntrue/
  rules.md          # Merged from all sources
  config.yaml
.cursor/rules/aligntrue.mdc
AGENTS.md
.aider.conf.yml     # Optional, can remove
```
...
2. `aligntrue sync`
3. Keep editing `.aligntrue/rules.md` or `.cursor/*.mdc` (two-way sync)
...
2. Review merged rules in `.aligntrue/rules.md`
3. `aligntrue sync`
