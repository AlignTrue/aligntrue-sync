...
| ------------------ | ---------------------------------------------------- | ---------- |
| `config.team.yaml` | Team settings (exporters, sources, lockfile mode)    | Committed  |
...
Team members joining will have `config.yaml` auto-created on first sync.
...
- Drift detection available (lockfile-only)
