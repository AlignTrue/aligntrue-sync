...
- Bundle imported rules into a catalog pack (no manifest needed)
...
- Optionally check **Bundle as pack** to create a shareable catalog pack that keeps rules together.
...
- Works for single files. Catalog packs are download-only today (no CLI install); add the individual rules from the downloaded zip if needed.
