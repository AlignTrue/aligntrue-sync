...
<Callout type="info">
  **Solo developer?** You can skip this page unless you're using external rule
  aligns. Overlays are for advanced customization of third-party rules.
</Callout>
...
└─ Need to change check logic or add new checks?
   └─ YES → Fork align or create custom align
```
...
# 3. Overlays are applied on each sync
```
...
Overlays modify the canonical IR (`.aligntrue/rules`) and affect **behavior** of checks and linting, even if the changes aren't visible in markdown exports.

**Example:**

- Overlay changes `severity: "warning"` → `severity: "error"`
- IR file `.aligntrue/rules` now has `severity: error`
...
cat .aligntrue/rules | grep -A 5 "your-rule-id"
...
- ✅ `.aligntrue/rules` contains the modified value
- ✅ Check behavior changes as expected

## Overlay metadata visibility

Overlays modify the IR (intermediate representation) metadata, but this metadata is not visible in all export formats. Understanding where overlay changes appear helps avoid confusion.
...
| `.aligntrue/rules` (IR)     | Always visible              | Contains modified metadata                   |
...
- security (./.aligntrue/rules/security.md): Security guidelines
```

These links point to the canonical rules but don't embed metadata like `severity`. The overlay still affects:

1. **Check behavior** - The IR has the modified severity
...
# 3. Check the IR directly
grep -A 5 "fingerprint: security" .aligntrue/rules/*.md
# Shows: severity: error (modified by overlay)
```
...
- [Solo Developer Guide](/docs/01-guides/01-solo-developer-guide) - Solo workflow with overlays
- [Team Guide](/docs/01-guides/02-team-guide) - Team collaboration with overlays
