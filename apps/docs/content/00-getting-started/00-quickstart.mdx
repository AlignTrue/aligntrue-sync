...
<Callout type="info">
**Where to edit:** Primary: `AGENTS.md` (you edit directly as markdown)

You can also edit agent files directly (`.cursor/rules/aligntrue.mdc`, etc.) and changes will automatically merge back on the next `aligntrue sync`.

Data flow:

```
AGENTS.md (primary) or any agent file
    ↓
aligntrue sync (detects edits)
    ↓
.aligntrue/.rules.yaml (internal IR, auto-generated)
    ↓
All agent files (exported and synced)
```

**Note:** The `.rules.yaml` file is internal and auto-generated—treat it like a build artifact. Always edit the markdown files.
...
- Learn [two‑way sync details](/docs/03-concepts/how-sync-actually-works)
- Configure [edit source](/docs/03-concepts/edit-source) and [.aligntrue/config.yaml](/docs/04-reference/config-reference)
