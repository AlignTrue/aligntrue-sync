...
See the [Glossary](/docs/03-concepts/glossary) for complete terminology and additional concepts like packs, bundles, and team mode.
...
**Important:** `.aligntrue/.rules.yaml` is auto-generated. Edit `AGENTS.md` or agent files instead.

AlignTrue keeps everything in sync automatically:

- Edit `AGENTS.md` or `.cursor/*.mdc` → sync updates all agents
- Never edit `.aligntrue/.rules.yaml` directly
...
The starter template uses natural markdown format with YAML frontmatter and section headings. For more details on authoring rules, see the [Natural Markdown Workflow guide](/docs/01-guides/natural-markdown-workflow) or the [Markdown authoring guide](/docs/04-reference/markdown-authoring) for the legacy fenced block format.
