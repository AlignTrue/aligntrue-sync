...
**Want to start with external rules?** Use `aligntrue init --source <git-url>` to import from GitHub, GitLab, SSH URLs, or local paths. See [Adding rules](/docs/01-guides/04-adding-rules) for all supported source formats.
...
**Want continuous auto-sync?** Run `aligntrue watch` to automatically sync on file changes. See the [CLI reference](/docs/04-reference/cli-reference).
