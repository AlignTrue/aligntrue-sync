...
Yes. Use a `.align.yaml` pack to bundle multiple files with preserved paths. You can share it via the CLI (`aligntrue init --source <repo-or-path>`) or the Catalog (paste the GitHub URL and download a zip). See [Align packs](/docs/03-concepts/align-packs) for manifest fields and limits.
