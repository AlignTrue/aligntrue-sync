...
Yes. Use a catalog pack to bundle multiple files with preserved paths. Create it in the catalog bulk import flow, then share the catalog link or download the zip. CLI installation for packs is not yet supported; add the downloaded rules individually if needed. See [Align packs](/docs/03-concepts/align-packs) for details.
