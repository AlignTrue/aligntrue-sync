...
**Don't edit `.aligntrue/.rules.yaml`** - This is auto-generated (like a build artifact). Think of it like `package-lock.json` - it exists for tooling, not for editing.
...
In solo mode, auto-pull is enabled by default, so changes flow both directions. In team mode, manually pull changes with `aligntrue sync --accept-agent`. See [Workflows guide](/docs/01-guides/01-workflows) for details.
