...
Then run `aligntrue sync` to fetch and apply the rules. First sync will prompt for privacy consent.
...
# Use in projects - add git source to config
cd project-1
# Edit .aligntrue/config.yaml to add:
# sources:
#   - type: git
#     url: ../shared-rules
#     ref: main
