...
export function validateScopePath(path: string): void {
  ...
  if (normalized.includes('..')) {
    throw new Error(`Invalid scope path "${path}": parent directory traversal (..) not allowed`)
  }
}
...
export function validateMergeOrder(order: MergeOrder): void {
  ...
  for (const item of order) {
    if (!validValues.has(item)) {
      throw new Error(`Invalid merge order value "${item}": must be one of root, path, local`)
    }
  }
 ...
 */
export function resolveScopes(workspacePath: string, config: ScopeConfig): ResolvedScope[] {
...
function matchFileToScope(file: string, scope: ResolvedScope): boolean {
  ...
  const scopePath = normalizedPath === '.' || normalizedPath === '' ? '' : normalizedPath
  if (scopePath && !file.startsWith(scopePath)) {
    return false
  }
  
  // If no include patterns, include everything in path
  const includeMatch = include.length === 0 || micromatch.isMatch(file, include)
...
function deepMergeRules(oldRule: AlignRule, newRule: AlignRule): AlignRule {
  ...
  if (newRule.vendor || oldRule.vendor) {
    ...
    const allKeys = new Set([...Object.keys(oldVendor), ...Object.keys(newVendor)])
