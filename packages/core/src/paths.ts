...
export function getAlignTruePaths(cwd: string = process.cwd()) {
  return {
    ...
    cursorRules: (scope: string): string => {
      ...
      const filename = scope === 'default' ? 'aligntrue' : scope.replace(/\//g, '-')
    ...
    privacyConsent: (): string => join(cwd, '.aligntrue', 'privacy-consent.json'),
    
    /** Path to telemetry events storage */
    telemetryEvents: (): string => join(cwd, '.aligntrue', 'telemetry-events.json'),
