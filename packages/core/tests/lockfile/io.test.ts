...
describe('lockfile I/O', () => {
  ...
  describe('writeLockfile', () => {
    ...
    it('sorts JSON keys', () => {
      ...
      const bundleHashLine = lines.findIndex(l => l.includes('"bundle_hash"'))
      const generatedAtLine = lines.findIndex(l => l.includes('"generated_at"'))
    ...
    it('overwrites existing file', () => {
      ...
      const lockfile2 = generateLockfile(
        { ...mockPack, rules: [{ ...mockPack.rules[0], guidance: 'Modified' }] },
        'team'
      )
    ...
    it('throws on write errors', () => {
      const lockfile = generateLockfile(mockPack, 'team')
      const invalidPath = '/invalid/path/.aligntrue.lock.json'
      
      expect(() => {
        writeLockfile(invalidPath, lockfile)
      }).toThrow() // Will throw ENOENT or similar error
    })
  })
