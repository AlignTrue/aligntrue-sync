...
describe("getImportSourcePath", () => {
  it("should return cursor path for cursor", () => {
    ...
    expect(path.replace(/\\/g, "/")).toBe("/test/workspace/.cursor/rules");
  });

  it("should return AGENTS.md path for universal agents", () => {
    // Normalize paths for cross-platform compatibility (Windows uses backslashes)
    expect(
      getImportSourcePath("copilot", "/test/workspace").replace(/\\/g, "/"),
    ).toBe("/test/workspace/AGENTS.md");
    expect(
      getImportSourcePath("claude-code", "/test/workspace").replace(/\\/g, "/"),
    ).toBe("/test/workspace/AGENTS.md");
    expect(
      getImportSourcePath("aider", "/test/workspace").replace(/\\/g, "/"),
    ).toBe("/test/workspace/AGENTS.md");
  });
