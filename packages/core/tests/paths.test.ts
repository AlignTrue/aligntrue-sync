...
describe("getAlignTruePaths", () => {
  ...
  it("should generate Cursor rules path for default scope", () => {
    const paths = getAlignTruePaths(testCwd);
    expect(paths.cursorRules("default")).toBe(
      join(testCwd, ".cursor", "rules", "aligntrue.mdc"),
    );
  });
  ...
  it("should normalize slashes in scope names", () => {
    const paths = getAlignTruePaths(testCwd);
    expect(paths.cursorRules("backend/api")).toBe(
      join(testCwd, ".cursor", "rules", "backend-api.mdc"),
    );
  });
  ...
  it("should generate privacy consent path", () => {
    const paths = getAlignTruePaths(testCwd);
    expect(paths.privacyConsent()).toBe(
      join(testCwd, ".aligntrue", "privacy-consent.json"),
    );
  });

  it("should generate telemetry events path", () => {
    const paths = getAlignTruePaths(testCwd);
    expect(paths.telemetryEvents()).toBe(
      join(testCwd, ".aligntrue", "telemetry-events.json"),
    );
  });
  ...
  it("should generate exporter output paths for vscode-mcp", () => {
    const paths = getAlignTruePaths(testCwd);
    expect(paths.exporterOutput("vscode-mcp", "mcp.json")).toBe(
      join(testCwd, ".vscode", "mcp.json"),
    );
  });

  it("should generate exporter output paths for amazonq", () => {
    const paths = getAlignTruePaths(testCwd);
    expect(paths.exporterOutput("amazonq", "rule.md")).toBe(
      join(testCwd, ".amazonq", "rules", "rule.md"),
    );
  });
