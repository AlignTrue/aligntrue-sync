/**
 * Firebase Studio exporter tests with mode hints support
 */

import { describe, it, expect, beforeEach, afterEach } from 'vitest'
import { readFileSync, rmSync, existsSync, mkdirSync } from 'fs'
import { join } from 'path'
import { FirebaseStudioExporter } from '../src/firebase-studio/index.js'
...
const TEST_OUTPUT_DIR = join(import.meta.dirname, 'temp-firebase-studio-test-output')

describe('FirebaseStudioExporter', () => {
  let exporter: FirebaseStudioExporter

  beforeEach(() => {
    exporter = new FirebaseStudioExporter()
  ...
  describe('Plugin Interface', () => {
    it('implements ExporterPlugin interface', () => {
      expect(exporter.name).toBe('firebase-studio')
  ...
  describe('Basic Export', () => {
    it('exports single rule to .idx/airules.md', async () => {
      ...
      expect(result.filesWritten[0]).toBe(join(TEST_OUTPUT_DIR, '.idx', 'airules.md'))
...
function createRequest(rules: AlignRule[], scope: ResolvedScope): ScopedExportRequest {
  return {
    scope,
    rules,
    outputPath: join(TEST_OUTPUT_DIR, '.idx', 'airules.md'),
  }
}
