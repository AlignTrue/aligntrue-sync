/**
 * AmazonQ exporter tests with mode hints support
 */
...
const TEST_OUTPUT_DIR = join(import.meta.dirname, 'temp-amazonq-test-output')

describe('AmazonQExporter', () => {
  let exporter: AmazonQExporter
  ...
  describe('Plugin Interface', () => {
    it('implements ExporterPlugin interface', () => {
      expect(exporter.name).toBe('amazonq')
  ...
  describe('Basic Export', () => {
    it('exports single rule to .amazonq/rules/ directory', async () => {
      ...
      expect(result.filesWritten[0]).toMatch(/\.amazonq\/rules\/rules\.md$/)
...
function createRequest(rules: AlignRule[], scope: ResolvedScope): ScopedExportRequest {
  return {
    scope,
    rules,
    outputPath: join(TEST_OUTPUT_DIR, '.amazonq', 'rules', 'rules.md'),
  }
}
