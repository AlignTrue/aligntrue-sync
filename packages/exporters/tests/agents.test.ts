...
describe("AgentsExporter", () => {
  ...
  describe("Format validation", () => {
    it("exports clean markdown without AlignTrue headers", async () => {
      const { sections } = loadFixture(FIXTURES_DIR, "single-rule.yaml");
      const scope = createMockScope();
      const request = createRequest(scope, sections);
      ...
      // Verify no "Generated by AlignTrue" header in output
      const outputPath = join(TEST_OUTPUT_DIR, "AGENTS.md");
      const content = await import("fs").then((fs) =>
        fs.promises.readFile(outputPath, "utf-8"),
      );
      expect(content).not.toContain("Generated by AlignTrue");
