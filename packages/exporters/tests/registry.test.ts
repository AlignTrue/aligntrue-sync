...
class TestExporter implements ExporterPlugin {
  ...
  async export(_request: ScopedExportRequest, _options: ExportOptions): Promise<ExportResult> {
    return {
      success: true,
...
describe('ExporterRegistry', () => {
  ...
  describe('programmatic registration', () => {
    ...
    it('throws if exporter missing name', () => {
      ...
      expect(() => registry.register(badExporter)).toThrow('must have name and version')
    })
  ...
  describe('manifest loading', () => {
    ...
    it('rejects manifest with missing required field (name)', () => {
      ...
      expect(() => registry.loadManifest(manifestPath)).toThrow('Invalid manifest')
      expect(() => registry.loadManifest(manifestPath)).toThrow('name')
    })

    it('rejects manifest with invalid semver', () => {
      const manifestPath = join(fixturesDir, 'invalid-manifest-bad-version.json')
      expect(() => registry.loadManifest(manifestPath)).toThrow('Invalid manifest')
      expect(() => registry.loadManifest(manifestPath)).toThrow('version')
    })

    it('rejects manifest with empty outputs array', () => {
      const manifestPath = join(fixturesDir, 'invalid-manifest-empty-outputs.json')
      expect(() => registry.loadManifest(manifestPath)).toThrow('Invalid manifest')
    })
  ...
  describe('handler loading', () => {
    ...
    it('throws on missing handler file', async () => {
      ...
      await expect(registry.loadHandler(handlerPath)).rejects.toThrow('Failed to load handler')
    })
  ...
  describe('adapter discovery', () => {
    it('discovers manifests in subdirectories', () => {
      ...
      expect(manifests.some(m => m.includes('mock-adapter/manifest.json'))).toBe(true)
    })
    ...
    it('throws on nonexistent search path', () => {
      ...
      expect(() => registry.discoverAdapters(badPath)).toThrow('Search path not found')
    })
