...
const TEST_OUTPUT_DIR = join(import.meta.dirname, 'temp-firebender-test-output')
...
describe('FirebenderExporter', () => {
  ...
  describe('Basic Export', () => {
    it('exports single rule to firebender.json', async () => {
      ...
      expect(result.filesWritten[0]).toBe(join(TEST_OUTPUT_DIR, 'firebender.json'))
  ...
  describe('mode hints integration', () => {
    ...
    it('should apply token caps and include warnings', async () => {
      const manyRules = Array(30).fill(null).map((_, i) => ({
        id: `test.rule.${i}`,
