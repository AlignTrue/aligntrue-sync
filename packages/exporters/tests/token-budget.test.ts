...
describe("globSpecificity", () => {
  it("scores deeper paths higher", () => {
    expect(globSpecificity("src/components/Button.tsx")).toBeGreaterThan(
      globSpecificity("src/*.tsx"),
    );
  });

  it("penalizes wildcards", () => {
    expect(globSpecificity("src/file.ts")).toBeGreaterThan(
      globSpecificity("src/*.ts"),
    );
  });
...
describe("estimateRuleTokens", () => {
  ...
  it("excludes hint text when includeHintText is false", () => {
    const rule: AlignRule = {
      ...
      mode: "intelligent",
      description:
        "Long description that would add many tokens to the hint text",
      guidance: "test",
      severity: "info",
    };
...
describe("prioritizeRulesForCapExport", () => {
  ...
  it("calculates total tokens accurately", () => {
    ...
    const manualSum = result.included.reduce(
      (sum, rule) => sum + estimateRuleTokens(rule, true),
      0,
    );
