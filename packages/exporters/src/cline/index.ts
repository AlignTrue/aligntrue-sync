...
export class ClineExporter implements ExporterPlugin {
  ...
  async export(request: ScopedExportRequest, options: ExportOptions): Promise<ExportResult> {
    ...
    const { content, warnings } = this.generateClineRulesContent(modeHints, maxBlocks, maxTokens)
  ...
  private generateClineRulesContent(modeHints: string, maxBlocks: number, maxTokens: number): { content: string; warnings: string[] } {
    const lines: string[] = []
