 ...
 */
export function estimateRuleTokens(rule: AlignRule, includeHintText: boolean = true): number {
  // JSON marker tokens
  const marker = {
    id: rule.id,
  ...
  return Math.ceil(markerTokens + hintTokens + titleTokens + guidanceTokens + descTokens)
}
