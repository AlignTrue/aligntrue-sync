...
export class ExporterRegistry {
  ...
  constructor() {
    ...
    const manifestSchema = JSON.parse(readFileSync(schemaPath, 'utf-8')) as AnySchemaObject
    ...
    try {
      ...
      if (!validate(manifest)) {
        const errors = validate.errors?.map((err: any) => {
          const missingProp = err.params && 'missingProperty' in err.params ? err.params['missingProperty'] : undefined
  ...
  async loadHandler(handlerPath: string): Promise<ExporterPlugin> {
    try {
      ...
      const ExporterClass = module.default || module.CursorExporter || module.AgentsMdExporter || module.VsCodeMcpExporter
      
      if (!ExporterClass) {
        throw new Error('Handler must export a default or named exporter class')
      }

      // Instantiate if it's a class
      const exporter = typeof ExporterClass === 'function' ? new ExporterClass() : ExporterClass

      if (!exporter.name || !exporter.version || !exporter.export) {
        throw new Error('Handler must export an ExporterPlugin-compatible class')
      }
  ...
  async registerFromManifest(manifestPath: string): Promise<void> {
    ...
    if (manifest.handler) {
      ...
      if (exporter.name !== manifest.name) {
        throw new Error(`Handler name "${exporter.name}" doesn't match manifest name "${manifest.name}"`)
      }
