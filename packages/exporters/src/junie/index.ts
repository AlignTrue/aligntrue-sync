...
export class JunieExporter implements ExporterPlugin {
  ...
  async export(request: ScopedExportRequest, options: ExportOptions): Promise<ExportResult> {
    ...
    const { content, warnings } = this.generateGuidelinesContent(modeHints, maxBlocks, maxTokens)
  ...
  private generateGuidelinesContent(modeHints: string, maxBlocks: number, maxTokens: number): { content: string; warnings: string[] } {
    const lines: string[] = []
  ...
  private computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has('check')) {
      notes.push('Machine-checkable rules (check) not represented in .junie/guidelines.md format')
    }
    if (unmappedFields.has('autofix')) {
      notes.push('Autofix hints not represented in .junie/guidelines.md format')
    }
