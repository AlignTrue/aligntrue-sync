...
export class OpenHandsExporter implements ExporterPlugin {
  name = 'openhands'
  version = '1.0.0'
  ...
  async export(request: ScopedExportRequest, options: ExportOptions): Promise<ExportResult> {
    ...
    const outputPath = join(outputDir, '.openhands', 'microagents', 'repo.md')

    const { modeHints, maxBlocks, maxTokens } = extractModeConfig(this.name, config)
    const { content, warnings } = this.generateRepoMdContent(modeHints, maxBlocks, maxTokens)
  ...
  private generateRepoMdContent(modeHints: string, maxBlocks: number, maxTokens: number): { content: string; warnings: string[] } {
    const lines: string[] = []

    lines.push('# Repository Microagents')
    lines.push('')
  ...
  private computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has('check')) {
      notes.push('Machine-checkable rules (check) not represented in .openhands/microagents/repo.md format')
    }
    if (unmappedFields.has('autofix')) {
      notes.push('Autofix hints not represented in .openhands/microagents/repo.md format')
    }
