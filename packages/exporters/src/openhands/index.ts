...
export class OpenHandsExporter implements ExporterPlugin {
  ...
  private computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has("check")) {
      notes.push(
        "Machine-checkable rules (check) not represented in .openhands/microagents/repo.md format",
      );
    }
    if (unmappedFields.has("autofix")) {
      notes.push(
        "Autofix hints not represented in .openhands/microagents/repo.md format",
      );
    }
