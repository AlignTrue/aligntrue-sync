...
export class CrushMdExporter implements ExporterPlugin {
  ...
  async export(request: ScopedExportRequest, options: ExportOptions): Promise<ExportResult> {
    ...
    const { content, warnings } = this.generateCrushMdContent(modeHints, maxBlocks, maxTokens)
  ...
  private generateCrushMdContent(modeHints: string, maxBlocks: number, maxTokens: number): { content: string; warnings: string[] } {
    const lines: string[] = []
  ...
  private computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has('check')) {
      notes.push('Machine-checkable rules (check) not represented in CRUSH.md format')
    }
