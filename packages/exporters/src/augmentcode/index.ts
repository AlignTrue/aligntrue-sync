...
export class AugmentCodeExporter extends ExporterBase {
  ...
  async export(
    ...
    if (!rules || rules.length === 0) {
      throw new Error(
        "AugmentCodeExporter requires at least one rule to export",
      );
    }
  ...
  override computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has("check")) {
      notes.push(
        "Machine-checkable rules (check) not represented in .augment/rules/ format",
      );
    }
    ...
    notes.push(
      "applies_to patterns preserved in metadata but not enforced by AugmentCode",
    );
