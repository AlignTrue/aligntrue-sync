...
export class WarpMdExporter extends ExporterBase {
  name = "warp-md";
  version = "1.0.0";
  ...
  async export(
    ...
    const { content, warnings } = this.generateWarpMdContent(
      modeHints,
      maxBlocks,
      maxTokens,
  ...
  override computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has("check")) {
      notes.push(
        "Machine-checkable rules (check) not represented in WARP.md format",
      );
    }
