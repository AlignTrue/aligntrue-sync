...
export class WarpMdExporter implements ExporterPlugin {
  ...
  async export(request: ScopedExportRequest, options: ExportOptions): Promise<ExportResult> {
    ...
    const { content, warnings } = this.generateWarpMdContent(modeHints, maxBlocks, maxTokens)
  ...
  private generateWarpMdContent(modeHints: string, maxBlocks: number, maxTokens: number): { content: string; warnings: string[] } {
    const lines: string[] = []
