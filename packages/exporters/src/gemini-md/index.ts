...
/**
 * GEMINI.md exporter
 * Exports AlignTrue rules to Gemini-specific GEMINI.md format
 *
 * Format: Single root-level GEMINI.md file (similar to AGENTS.md but Gemini-specific)
 */
...
export class GeminiMdExporter extends ExporterBase {
  name = "gemini-md";
  version = "1.0.0";
  ...
  async export(
    ...
    const { content, warnings } = this.generateGeminiMdContent(
      modeHints,
      maxBlocks,
      maxTokens,
  ...
  private generateGeminiMdContent(
    modeHints: ModeHints,
    maxBlocks: number,
    maxTokens: number,
    ...
    lines.push("");
    lines.push("This file contains rules and guidance for Google Gemini.");
  ...
  override computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has("check")) {
      notes.push(
        "Machine-checkable rules (check) not represented in GEMINI.md format",
      );
    }
    if (unmappedFields.has("autofix")) {
      notes.push("Autofix hints not represented in GEMINI.md format");
    }
