...
export class GooseExporter implements ExporterPlugin {
  ...
  async export(request: ScopedExportRequest, options: ExportOptions): Promise<ExportResult> {
    ...
    const { content, warnings } = this.generateGooseHintsContent(modeHints, maxBlocks, maxTokens)
  ...
  private generateGooseHintsContent(modeHints: string, maxBlocks: number, maxTokens: number): { content: string; warnings: string[] } {
    const lines: string[] = []
  ...
  private computeFidelityNotes(rules: AlignRule[]): string[] {
    ...
    if (unmappedFields.has('check')) {
      notes.push('Machine-checkable rules (check) not represented in .goosehints format')
    }
