 ...
 */
export function canonicalizeJson(obj: unknown, excludeVolatile: boolean = true): string {
...
function filterVolatileVendorFields(data: unknown): unknown {
  ...
  if (data && typeof data === 'object') {
    ...
    if (obj['vendor'] && typeof obj['vendor'] === 'object' && obj['vendor']['_meta']?.['volatile']) {
 ...
 */
function filterVolatilePaths(vendor: Record<string, any>, volatilePaths: string[]): Record<string, any> {
  const result: Record<string, any> = {}
...
export function computeAlignHash(alignInput: string | unknown): string {
  ...
  const obj = typeof alignInput === 'string' 
