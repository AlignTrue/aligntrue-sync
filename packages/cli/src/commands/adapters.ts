...
async function listAdapters(): Promise<void> {
  ...
  for (const adapter of adapters) {
    const icon = adapter.status === 'installed' ? '✓' : adapter.status === 'available' ? '-' : '❌'
    const nameWidth = 24
...
async function enableAdapters(args: string[], interactive?: boolean): Promise<void> {
  ...
  if (interactive) {
    ...
    const selected = await clack.multiselect({
      ...
      options: choices,
      initialValues: choices.filter(c => currentlyEnabled.has(c.value)).map(c => c.value),
      required: true
    })
    ...
    if (adapter.status === 'invalid') {
      ...
      console.error('  This adapter is not available in the installed exporters package')
      process.exit(1)
    }
  ...
  recordEvent({ 
    ...
    align_hashes_used: [],
    ...(adaptersToEnable.length > 0 && { export_target: adaptersToEnable.join(',') })
  })

  if (adaptersToEnable.length === 1) {
    console.log(`✓ Enabled adapter: ${adaptersToEnable[0]}`)
  } else {
    console.log(`✓ Enabled ${addedCount} adapter(s): ${adaptersToEnable.join(', ')}`)
  }
...
async function disableAdapter(args: string[]): Promise<void> {
  ...
  if (currentExporters.length === 1) {
    ...
    console.error('  Enable another adapter first: aligntrue adapters enable <adapter>')
    process.exit(1)
  }
