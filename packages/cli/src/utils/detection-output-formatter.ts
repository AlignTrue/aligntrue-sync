...
export function shouldRecommendEditSourceSwitch(
  ...
  // Check if any new agent is multi-file
  ...
  // Check if current source is single-file
  ...
  const isSingleFile = currentSources.some((src) =>
    singleFilePatterns.some((pattern) => src.includes(pattern)),
  );
