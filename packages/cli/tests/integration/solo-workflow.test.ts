...
describe('Solo Workflow Integration', () => {
  describe('Fresh Install', () => {
    it('completes setup in minimal time with minimal config', () => {
      ...
      const cursorPath = join(TEST_DIR, '.cursor', 'rules', 'aligntrue-starter.mdc')
      ...
      writeFileSync(cursorPath, '## Rule: test-rule\n\n**Severity:** error\n**Applies to:** `**/*.ts`\n\nTest guidance\n', 'utf-8')
    ...
    it('creates native format starter (not YAML IR)', () => {
      ...
      mkdirSync(join(TEST_DIR, '.cursor', 'rules'), { recursive: true })
      writeFileSync(cursorPath, '## Rule: typescript-strict\n\n**Severity:** error\n**Applies to:** `tsconfig.json`\n\nEnable strict mode\n', 'utf-8')
...
`
  ...
  describe('Zero YAML Interaction', () => {
    it('solo dev workflow never requires editing YAML IR', () => {
      ...
      writeFileSync(cursorPath, '## Rule: test\n\n**Severity:** error\n**Applies to:** `**/*`\n\nGuidance\n', 'utf-8')
      ...
      const updatedContent = readFileSync(cursorPath, 'utf-8') + '\n## Rule: test2\n\n**Severity:** warn\n\nMore guidance\n'
