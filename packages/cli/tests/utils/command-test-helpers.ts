 ...
 * ```
 */
export function mockCommandArgs(overrides: {
  help?: boolean
  dryRun?: boolean
  force?: boolean
  config?: string
  ...
  return {
    start() {
      ...
      process.stdout.write = ((chunk: unknown, ...args: unknown[]): boolean => {
        ...
        return originalStdoutWrite.call(process.stdout, chunk as Buffer, ...args as [BufferEncoding?, ((err?: Error) => void)?])
      ...
      process.stderr.write = ((chunk: unknown, ...args: unknown[]): boolean => {
        const str = chunk?.toString() || ''
        stderrData += str
        return originalStderrWrite.call(process.stderr, chunk as Buffer, ...args as [BufferEncoding?, ((err?: Error) => void)?])
