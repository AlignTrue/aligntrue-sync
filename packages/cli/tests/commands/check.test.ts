...
describe('check command', () => {
  ...
  describe('help and argument parsing', () => {
    it('shows help when --help flag is provided', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('aligntrue check'))
    ...
    it('requires --ci flag for now', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('--ci flag is required'))
  ...
  describe('config validation', () => {
    it('exits with code 2 when config not found', async () => {
      vi.mocked(core.loadConfig).mockRejectedValue(new Error('Config file not found: .aligntrue/config.yaml'))
  ...
  describe('lockfile validation - solo mode', () => {
    ...
    it('skips lockfile validation in solo mode', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('skipped (solo mode)'))
      expect(exitSpy).toHaveBeenCalledWith(0)
    })
  })
  ...
  describe('lockfile validation - team mode', () => {
    ...
    it('exits with code 1 when lockfile missing in team mode', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Lockfile not found'))
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
    ...
    it('exits with code 1 on lockfile hash mismatch', async () => {
      const mockLockfile = {
        ...
        mode: 'soft' as const,
        rules: [
          { rule_id: 'test.rule', content_hash: 'abc123' },
        ],
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Lockfile drift detected'))
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('test.rule'))
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
    ...
    it('exits with code 1 when new rules detected', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('New rules not in lockfile'))
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('test.rule.new'))
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
    ...
    it('exits with code 1 when deleted rules detected', async () => {
      const mockLockfile = {
        ...
        mode: 'soft' as const,
        rules: [
          { rule_id: 'test.rule.deleted', content_hash: 'abc123' },
        ],
        bundle_hash: 'bundle123',
      }
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Rules in lockfile but not in IR'))
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('test.rule.deleted'))
      expect(exitSpy).toHaveBeenCalledWith(1)
    })
    ...
    it('passes validation when lockfile matches', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Validation passed'))
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('matches current rules'))
      expect(exitSpy).toHaveBeenCalledWith(0)
    })
    ...
    it('exits with code 2 on lockfile read error', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Lockfile validation failed'))
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to parse lockfile'))
      expect(exitSpy).toHaveBeenCalledWith(2)
    })
  })
  ...
  describe('error handling', () => {
    it('exits with code 2 on unexpected system error', async () => {
      vi.mocked(core.loadConfig).mockRejectedValue(new Error('Unexpected filesystem error'))
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('System error'))
      expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Unexpected filesystem error'))
      expect(exitSpy).toHaveBeenCalledWith(2)
    })
  })
})
