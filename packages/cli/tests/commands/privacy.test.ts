...
describe('privacy command', () => {
  ...
  describe('audit command', () => {
    it('shows message when no consents', async () => {
      ...
      expect(clack.log.info).toHaveBeenCalledWith('No privacy consents granted yet');
      expect(consoleSpy).toHaveBeenCalled();
    ...
    it('lists all granted consents', async () => {
      ...
      const manager = createConsentManager(join(testDir, '.aligntrue/privacy-consent.json'));
  ...
  describe('revoke command', () => {
    ...
    it('shows info when no consent to revoke', async () => {
      ...
      expect(clack.log.info).toHaveBeenCalledWith("No consent for 'catalog' to revoke");
      consoleSpy.mockRestore();
    });
    ...
    it('revokes specific operation', async () => {
      ...
      expect(clack.log.success).toHaveBeenCalledWith("Revoked consent for 'catalog'");
    ...
    it('revokes all consents when --all flag used', async () => {
      ...
      expect(clack.log.success).toHaveBeenCalledWith('Revoked all consents (2)');
