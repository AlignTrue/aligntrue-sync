...
describe('adapters command', () => {
  ...
  const createConfig = (exporters: string[] = ['cursor', 'agents-md']) => {
    ...
    writeFileSync('.aligntrue/config.yaml', `version: '1'\nmode: solo\nexporters:\n${exporters.map(e => `  - ${e}`).join('\n')}\n`)
  }
  ...
  describe('enable subcommand', () => {
    it('enables a single adapter', async () => {
      ...
      await expect(adapters(['enable', 'agents-md'])).rejects.toThrow('process.exit: 0')
      expect(exitCode).toBe(0)
    ...
    it('shows friendly message if already enabled', async () => {
      ...
      await expect(adapters(['enable', 'cursor'])).rejects.toThrow('process.exit: 0')
      expect(exitCode).toBe(0)
    })
    ...
    it('shows error for invalid adapter', async () => {
      ...
      await expect(adapters(['enable', 'nonexistent'])).rejects.toThrow('process.exit: 1')
      expect(exitCode).toBe(1)
    })
    ...
    it('enables multiple adapters in interactive mode', async () => {
      ...
      vi.mocked(clack.multiselect).mockResolvedValue(['cursor', 'agents-md', 'claude-md'])
      
      await expect(adapters(['enable', '--interactive'])).rejects.toThrow('process.exit: 0')
      expect(exitCode).toBe(0)
    ...
    it('sorts exporters alphabetically', async () => {
      ...
      await expect(adapters(['enable', 'zed-config'])).rejects.toThrow('process.exit: 0')
      await expect(adapters(['enable', 'agents-md'])).rejects.toThrow('process.exit: 0')
      
      const config = readFileSync('.aligntrue/config.yaml', 'utf-8')
      const lines = config.split('\n')
      const exportersIndex = lines.findIndex(l => l.includes('exporters:'))
      const exportersList = lines.slice(exportersIndex + 1).filter(l => l.trim().startsWith('- '))
  ...
  describe('disable subcommand', () => {
    it('disables an adapter', async () => {
      ...
      await expect(adapters(['disable', 'cursor'])).rejects.toThrow('process.exit: 0')
      expect(exitCode).toBe(0)
    ...
    it('shows error if adapter not enabled', async () => {
      ...
      await expect(adapters(['disable', 'agents-md'])).rejects.toThrow('process.exit: 1')
      expect(exitCode).toBe(1)
    })
    ...
    it('prevents disabling last adapter', async () => {
      ...
      await expect(adapters(['disable', 'cursor'])).rejects.toThrow('process.exit: 1')
      expect(exitCode).toBe(1)
    })
  ...
  describe('error handling', () => {
    it('handles missing config for enable', async () => {
      await expect(adapters(['enable', 'cursor'])).rejects.toThrow('process.exit: 1')
      expect(exitCode).toBe(1)
    })
