...
describe('Conformance: Check Vectors', () => {
  const checkTypes = ['file-presence', 'path-convention', 'manifest-policy', 'regex', 'command-runner'];
...
describe('Conformance: Golden Packs', () => {
  const goldenFiles = readdirSync(goldenDir).filter(f => f.endsWith('.aligntrue.yaml'));
...
describe('Conformance: Summary', () => {
  it('reports total vector counts', () => {
    ...
    console.log(`  Total: ${canonVectors.length + checkVectorCount + goldenFiles.length}`);
