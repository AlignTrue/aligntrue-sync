...
describe('CatalogProvider - Fetch Flow', () => {
  ...
  it('throws error when pack ID not in catalog index', async () => {
    ...
    await expect(
      provider.fetch('packs/nonexistent/pack')
    ).rejects.toThrow(/Pack not found in catalog: packs\/nonexistent\/pack/)
  })
  ...
  it('handles HTTP 404 error with clear message', async () => {
    ...
    await expect(
      provider.fetch('packs/base/base-global')
    ).rejects.toThrow(/Pack YAML not found.*may have been removed/s)
  })
  ...
  it('handles empty YAML response', async () => {
    ...
    await expect(
      provider.fetch('packs/base/base-global')
    ).rejects.toThrow(/Empty YAML content/)
  })
  ...
  it('handles invalid catalog index structure', async () => {
    ...
    await expect(
      provider.fetch('packs/base/base-global')
    ).rejects.toThrow(/Invalid catalog index structure/)
  })
})
...
describe('CatalogProvider - Cache Operations', () => {
  ...
  it('writes cache atomically (temp + rename)', async () => {
    ...
    const packTempPath = join(TEST_CACHE_DIR, 'packs/base/base-global.yaml.tmp')
  ...
  it('handles corrupted cache index gracefully', async () => {
    ...
    writeFileSync(join(TEST_CACHE_DIR, 'index.json'), 'corrupted json {', 'utf-8')
...
describe('CatalogProvider - Offline Fallback', () => {
  ...
  it('throws error when no cache available and network fails', async () => {
    ...
    await expect(
      provider.fetch('packs/base/base-global')
    ).rejects.toThrow(/Failed to fetch pack.*no cache available/)
  })
...
describe('CatalogProvider - Security', () => {
  ...
  it('rejects pack IDs with parent directory traversal', async () => {
    ...
    await expect(
      provider.fetch('packs/../../../etc/passwd')
    ).rejects.toThrow(/Invalid pack ID.*cannot contain/s)
  })
  ...
  it('rejects absolute paths in pack IDs', async () => {
    ...
    ).rejects.toThrow(/Invalid pack ID.*must be relative/s)
  })
  ...
  it('rejects Windows absolute paths in pack IDs', async () => {
    ...
    await expect(
      provider.fetch('C:\\Windows\\System32')
  ...
  it('validates pack ID format', async () => {
    ...
    await expect(
      provider.fetch('invalid-format')
    ).rejects.toThrow(/Invalid pack ID format.*Expected format/s)
  })
