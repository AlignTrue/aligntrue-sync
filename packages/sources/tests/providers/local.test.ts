...
const TEST_DIR = join(
  process.cwd(),
  "packages/sources/tests/temp-local-provider",
);
...
describe("LocalProvider", () => {
  ...
  describe("fetch", () => {
    ...
    it("throws on file not found", async () => {
      ...
      await expect(provider.fetch("nonexistent.txt")).rejects.toThrow(
        /File not found/,
      );
    });
    ...
    it("prevents path traversal attacks", async () => {
      ...
      await expect(provider.fetch("../../sensitive.txt")).rejects.toThrow(
        /path traversal/,
      );
      await expect(provider.fetch("test/../../../etc/passwd")).rejects.toThrow(
        /path traversal/,
      );
    });
