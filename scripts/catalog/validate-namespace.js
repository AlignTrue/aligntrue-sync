 ...
 */
export function loadNamespaceRegistry(registryPath) {
 ...
 */
export function matchesNamespace(packId, pattern) {
  // Exact match
  if (packId === pattern) {
    return true;
  }
 ...
 */
export function findNamespaceOwner(packId, registry) {
 ...
 */
export function validateNamespace(packId, claimedOwner, registry) {
 ...
 */
export function extractOrgFromPackId(packId) {
  const parts = packId.split("/");
  if (parts.length >= 2 && parts[0] === "packs") {
    return parts[1];
  }
